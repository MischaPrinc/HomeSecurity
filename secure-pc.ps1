...
# ==============================================================================
#         G E T / S E T   -   A D V A N C E D   S E C U R I T Y
# ==============================================================================

function Get-CredentialGuardStatus {
    try {
        # Kontrola, zda je VBS (Virtualization-based security) zapnuto
        ...
        if ($null -eq $vbs -or $vbs.EnableVirtualizationBasedSecurity -ne 1) {
            return "VBS neni aktivni"
        }

        # Kontrola konfigurace Credential Guard
        $cg = Get-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Control\Lsa" -Name "LsaCfgFlags" -ErrorAction SilentlyContinue
        if ($null -eq $cg) {
            return "Nenastaveno"
        }

        # Hodnota 1 = Zapnuto s UEFI zamykanim, 2 = Zapnuto bez zamykani
        if ($cg.LsaCfgFlags -eq 1) { "Zapnuto (s UEFI zamykanim)" }
        elseif ($cg.LsaCfgFlags -eq 2) { "Zapnuto (bez zamykani)" }
        else { "Vypnuto" }
    } catch { "Neznamy" }
}

function Set-CredentialGuardState {
    param([bool]$Enabled)
    Write-Host ""
    if ($Enabled) {
        Write-Host "  -- Zapinani Credential Guard --" -ForegroundColor Cyan
        try {
            # Krok 1: Povolit Virtualization Based Security (VBS)
            ...
            Write-Host "  [OK] VBS povolen." -ForegroundColor Green

            # Krok 2: Povolit Credential Guard s UEFI zamykanim pro nejvyssi bezpecnost
            $lsaPath = "HKLM:\SYSTEM\CurrentControlSet\Control\Lsa"
            if (-not (Test-Path $lsaPath)) { New-Item -Path $lsaPath -Force | Out-Null }
            Set-ItemProperty -Path $lsaPath -Name "LsaCfgFlags" -Value 1 -Type DWord -ErrorAction Stop
            Write-Host "  [OK] Credential Guard nastaven na 'Zapnuto s UEFI zamykanim'." -ForegroundColor Green
            Write-Host ""
            Write-Host "  !!! DULEZITE: Pro aktivaci Credential Guard je nutny RESTART pocitace." -ForegroundColor Yellow
            Write-Host "  Po restartu overte stav v tomto skriptu nebo v 'msinfo32.exe' (Systemove informace)." -ForegroundColor Yellow
        } catch {
            Write-Host "  [CHYBA] Nepodarilo se nastavit registry pro Credential Guard: $_" -ForegroundColor Red
            Write-Host "  Ujistete se, ze virtualizace (Intel VT-x/AMD-V) je povolena v BIOS/UEFI." -ForegroundColor Yellow
        }
    } else {
        Write-Host "  -- Vypinani Credential Guard --" -ForegroundColor Cyan
        try {
            Set-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Control\Lsa" -Name "LsaCfgFlags" -Value 0 -Type DWord -ErrorAction Stop
            Write-Host "  [OK] Credential Guard nastaven na 'Vypnuto'." -ForegroundColor Green
            Write-Host "  !!! DULEZITE: Pro deaktivaci je nutny RESTART pocitace." -ForegroundColor Yellow
        } catch {
            Write-Host "  [CHYBA] Nepodarilo se nastavit registry: $_" -ForegroundColor Red
        }
    }
}
...
function Get-BitLockerStatus {
    try {
        ...
        if (-not $volumes) {
            return "Nelze ziskat informace (sluzba mozna nebezi?)"
        }
        $statuses = foreach ($vol in $volumes) {
            $status = switch ($vol.VolumeStatus) {
                "FullyEncrypted" { "Plne sifrovan" }
                "FullyDecrypted" { "Desifrovan" }
                "EncryptionInProgress" { "Sifruje se" }
                "DecryptionInProgress" { "Desifruje se" }
                default { $vol.VolumeStatus }
            }
            $color = if ($status -eq "Plne sifrovan") { 'Green' } else { 'Red' }
            @{
...
function Show-FullStatus {
    ...
    $v = Get-CredentialGuardStatus;   Write-Status "Credential Guard"            $v (Get-StatusColor $v)
    $blStatus = Get-BitLockerStatus
    if ($blStatus -is [array]) {
        foreach ($item in $blStatus) {
            Write-Status "BitLocker ($($item.MountPoint))" $item.Status $item.Color
        }
    } else {
        Write-Status "BitLocker" $blStatus 'Yellow'
    }
...
function Enable-MaxSecurity {
    ...
    Write-Host ""
    Write-Host "  -- Pokrocile zabezpeceni --" -ForegroundColor Cyan
    Set-CredentialGuardState -Enabled $true
    Write-Host ""
    Write-Host "  *** Vse nastaveno na maximalni ochranu! ***" -ForegroundColor Green
    Write-Host "  !!! NEZAPOMENTE RESTARTOVAT POCITAC pro aplikaci vsech zmen (Credential Guard)." -ForegroundColor Yellow
}
...
"@
        ...
        switch ($c) {
            ...
            "0" { return }
            default { Write-Host "  Neplatna volba." -ForegroundColor Red; Start-Sleep 1 }
  -- 7) Pokrocile zabezpeceni -------------------------------------------------
function Show-Menu-AdvancedSecurity {
    do {
        Show-Banner
        Write-SubHeader "POKROCILE ZABEZPECENI" @"
  Credential Guard pouziva virtualizaci k izolaci a ochrane hesel a
  dalsich prihlasovacich udaju, coz vyrazne ztezuje jejich kradez.
  BitLocker sifruje cely disk a chrani tak data pri ztrate/kradezi.
  POZOR: Zapnuti Credential Guard vyzaduje restart!
"@

        $cgSt = Get-CredentialGuardStatus
        Write-Host "  Aktualni stav:" -ForegroundColor DarkGray
        Write-Host "    Credential Guard: $cgSt" -ForegroundColor DarkGray
        Write-Host ""
        Write-Host "    Stav BitLockeru:" -ForegroundColor DarkGray
        $blStatus = Get-BitLockerStatus
        if ($blStatus -is [array]) {
            foreach ($item in $blStatus) {
                Write-Host "      $($item.MountPoint.PadRight(5)):" -NoNewline
                Write-Host " $($item.Status)" -ForegroundColor $item.Color
            }
        } else {
            Write-Host "      $blStatus" -ForegroundColor 'Yellow'
    $cgQ  = Get-CredentialGuardStatus
    ...
    $cgC  = if ($cgQ -match "Zapnuto") { 'Green' } elseif ($cgQ -match "Vypnuto") { 'Red' } else { 'Yellow' }
    ...
    Write-Host "    7)  Pokrocile zabezpeceni" -ForegroundColor White -NoNewline
    Write-Host "    [CG: " -NoNewline -ForegroundColor DarkGray
    Write-Host "$cgQ" -NoNewline -ForegroundColor $cgC
    Write-Host "]" -ForegroundColor DarkGray
    Write-Host ""
    ...
    switch ($mainChoice) {
        ...
        "5"  { Show-Menu-Sysmon }
        "6"  { Show-Menu-DNS }
        "7"  { Show-Menu-AdvancedSecurityndColor DarkGray
    Write-Host "        ASR: $asrQ" -ForegroundColor DarkGray
    Write-Host ""
